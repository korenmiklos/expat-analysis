...
gr export "$outputdir\selection\hist_tenure.pdf", replace
*CEO max tenure
...
gr export "$outputdir\selection\hist_tenuremax.pdf", replace
restore
...
gr export "$outputdir\selection\hist_nceo.pdf", replace
restore
...
global descvar emp lnQ lnQL TFP_cd exporter_5
...
esttab d1 d2 d3 d4 using "$outputdir\selection\desc_firmvars_1.csv", main(mean) mtitle("Domestic" "Incumbent" "Local" "Expat") label pare replace

esttab d1 d2 d3 d4 using "$outputdir\selection\desc_firmvars_1.csv", main(mean) aux(sd) mtitle("Domestic" "Incumbent" "Local" "Expat") label pare replace
eststo clear
...
gr export "$outputdir\selection\g_selection_emp.pdf", replace
...
gr export "$outputdir\selection\g_selection_TFP.pdf", replace
...
gr export "$outputdir\selection\g_selection_export.pdf", replace
...
esttab est1 est2 est3 using "$outputdir\selection\selection_regression.tex", star(* .1 ** .05 *** .01) b(3) keep(lnK sales_gr lnQL exporter_5) ///
...
foreach X of varlist $outcome_perf {
	eststo: qui reghdfe `X' foreign [aw = inverse_weight], a($fixed_effects) cluster(id_man_firm_spell)
}
esttab using "$outputdir\effect\foreign_effect_1.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(foreign) noconstant nonote replace label
eststo clear

*Direct effect
foreach X of varlist $outcome_perf {
	eststo: qui reghdfe `X' foreign during during_foreign during_expat [aw = inverse_weight], a($fixed_effects) cluster(id_man_firm_spell)
}
...
esttab using "$outputdir\effect\direct_effect_1_ten.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep($report) noconstant nonote label replace 
eststo clear
...
esttab using "$outputdir\effect\direct_effect_2.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep($report) noconstant nonote replace label
eststo clear
...
esttab using "$outputdir\effect\direct_effect_trade.tex", r2 star(* .11 ** .05 *** .01) se b(3) keep($report) noconstant nonote replace label
eststo clear
...
esttab using "$outputdir\effect\long_effect_1.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep($report after_foreign after_expat) noconstant nonote replace label
eststo clear
...
esttab using "$outputdir\effect\long_effect_2.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep($report after_foreign after_expat) noconstant nonote replace label
eststo clear
...
esttab using "$outputdir\effect\switch_effect_1.tex", r2 star(* .1 ** .05 *** .01) se b(3) noconstant nonote replace label ///
...
esttab using "$outputdir\effect\direct_effect_1_switch.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(foreign $switch_type_1) noconstant nonote label replace 
eststo clear

foreach X of varlist $outcome_input {
	eststo: qui reghdfe `X' foreign $switch_type_1 during [aw = inverse_weight] if $sample_acquisition_1, a($fixed_effects) cluster(frame_id)
}
esttab using "$outputdir\effect\direct_effect_input_switch.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(foreign $switch_type_1) noconstant nonote label replace 
eststo clear

foreach X of varlist $outcome_trade {
	eststo: qui reghdfe `X' foreign $switch_type_1 during [aw = inverse_weight] if $sample_acquisition_1, a($fixed_effects) cluster(frame_id)
}
esttab using "$outputdir\effect\direct_effect_trade_switch.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(foreign $switch_type_1) noconstant nonote label replace 
eststo clear
...
foreach var in $outcome_perf {
	...
	qui reghdfe `var' foreign $dynamics [aw = inverse_weight] if $sample_acquisition_1 & tenure <= 5, a($fixed_effects) cluster(frame_id)
...
foreach var in $outcome_perf {
	...
	*gr export "$outputdir\effect\gr_`var'_expat.pdf", replace 
	...
	gr export "$outputdir\effect\gr_`var'.pdf", replace 
...
foreach var2 in DD ED DE EE {
	...
	gr export "$outputdir\effect\gr_`var'_`var2'.pdf", replace 
...
esttab using "$outputdir\effect\dynamic_effect_10.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(before_4 before_3 before_2 before_1 ///
during_0 during_1 during_2 during_3 during_4 during_5) ///
noconstant nonote alignment(D{.}{.}{-1}) replace 
esttab using "$outputdir\effect\dynamic_effect_11.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(before_foreign_* during_foreign_*) ///
noconstant nonote alignment(D{.}{.}{-1}) replace 
esttab using "$outputdir\effect\dynamic_effect_12.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(before_expat_* during_expat_*) ///
noconstant nonote alignment(D{.}{.}{-1}) replace 
eststo clear
...
esttab using "$outputdir\effect\switch_effect_dyn_1.tex", r2 star(* .1 ** .05 *** .01) se b(3) noconstant nonote alignment(D{.}{.}{-1}) replace label ///
keep(foreign $switch_type)
eststo clear
...
esttab using "$outputdir\effect\foreign_effect_2.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(foreign) noconstant nonote alignment(D{.}{.}{-1}) replace label
eststo clear
...
esttab using "$outputdir\effect\direct_effect_2.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep($report) noconstant nonote alignment(D{.}{.}{-1}) replace 
eststo clear
...
esttab using "$outputdir\effect\long_effect_2.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(foreign during* after*) noconstant nonote alignment(D{.}{.}{-1}) replace 
eststo clear

foreach X of varlist $outcomes_2 {
	eststo: qui reghdfe `X' foreign DD $switch_type [aw = inverse_weight] if $sample_acquisition, a($fixed_effects) cluster(frame_id)
}
esttab using "$outputdir\effect\switch_effect_2.tex", r2 star(* .1 ** .05 *** .01) se b(3) noconstant nonote alignment(D{.}{.}{-1}) replace 
eststo clear
...
esttab using "$outputdir\effect\dynamic_effect_20.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(before_4 before_3 before_2 before_1 ///
during_0 during_1 during_2 during_3 during_4 during_5) ///
noconstant nonote alignment(D{.}{.}{-1}) replace 
esttab using "$outputdir\effect\dynamic_effect_21.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(before_foreign_* during_foreign_*) ///
noconstant nonote alignment(D{.}{.}{-1}) replace 
esttab using "$outputdir\effect\dynamic_effect_22.tex", r2 star(* .1 ** .05 *** .01) se b(3) keep(before_expat_* during_expat_*) ///
noconstant nonote alignment(D{.}{.}{-1}) replace
eststo clear
