...
global datadir "C:\Users\Almos\Documents\Research\Expat\New_data_version\temp" 
...
global sample_event before & during
...
global fixed_effects id_man_firm_spell teaor08_2d##year foundyear
...
tempvar n N1
gen `n' = 1
bysort frame_id_numeric year: egen `N1' = sum(`n')
gen inverse_weight = 1/`N1'

*New identifier
egen id_man_firm_spell = group(manager_id frame_id_numeric job_spell)

xtset id_man_firm_spell year
...
bysort id_man_firm_spell: egen teaor_mode = mode(teaor08_2d), maxmode
...
*Cobb-Douglas
keep if $sample_acquisition_1
egen x = tag(id_man_firm_spell year)
keep if x 
...
foreach l of local levels {
  disp `l'
  qui reghdfe lnQ lnL lnK lnM [aw = inverse_weight] if teaor_mode == `l', a(id_man_firm_spell year foundyear) resid
...
*TODO: this is computed only for the foreign period. Should be extended to the whole period of the manager.
bysort frame_id_numeric: egen x = min(job_begin) if foreign
gen foreign_hire_first = (x == job_begin) 
