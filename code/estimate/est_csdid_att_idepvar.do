...
cap log close
log using "`here'/output/est_csdid_att_idepvar", text replace
...
foreach depvar in lnQL lnK lnL exporter lnQ TFP_cd lnIK_0 lnQh lnQhr {
	foreach var in fy_foreign_o fy_foreign_ho fy_expat {
		csdid `depvar' lnL_sq lnQ_sq lnK_sq exporter_before i.year i.teaor08_2d, ivar(frame_id_numeric) time(year) gvar(`var') agg(simple) method(reg) notyet
...
esttab m* using "`here'/output/table_csdid_att_idepvar.tex", mtitle b(3) se(3) replace
esttab m* using "`here'/output/table_csdid_att_idepvar.txt", mtitle b(3) se(3) replace
