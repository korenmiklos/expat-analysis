...
gen young=(age<6)
gen medium=(age>5 & age<11)
gen so=(so2_with_mo2==1 | so3==1)
egen ind1=group(teaor08_1d)
...
*Check limitcontrol, treatment2
csdid2 TFP_cd,  ivar(frame_id_numeric) tvar(year) gvar(first_expat)
estat event, revent(-5/5)

attgt TFP_cd, treatment(has_expat_ceo) aggregate(att) pre(5) post(5) notyet
attgt TFP_cd, treatment(has_expat_ceo) aggregate(att) pre(5) post(5) notyet limitcontrol(foreign==0)

preserve
keep if ever_foreign_hire==1
attgt lnQd,treatment(has_expat_ceo) aggregate(att) pre(5) post(5) treatment2(foreign_hire)
restore
