...
cap log close
log using "`here'/output/est_attgt_IK", text replace
...
foreach sample in ef efh {
	foreach var in foreign foreign_hire has_expat {
		...
		*attgt exporter lnQL if inrange(year, 1992, 2004) & ever_foreign, treatment(`var') aggregate(e) pre(2) post(3) reps(20)
		attgt lnIK if `sample', treatment(`var') aggregate(e) pre(4) post(4) reps(20) notyet
		count if e(sample) == 1
...
esttab model* using "`here'/output/table_attgt_IK.tex", mtitle b(3) se(3) replace
esttab model* using "`here'/output/table_attgt_IK.txt", mtitle b(3) se(3) replace
