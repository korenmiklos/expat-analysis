...
cap log close
log using "`here'/output/est_csdid_att_ipw", text replace
...
foreach depvar in lnQL lnK lnL exporter lnQ TFP_cd lnIK_0 lnQh lnQhr {
	foreach var in fy_foreign_o fy_foreign_ho fy_expat {
		csdid `depvar', ivar(frame_id_numeric) time(year) gvar(`var') agg(simple) method(ipw) notyet
...
esttab m* using "`here'/output/table_csdid_att_ipw.tex", mtitle b(3) se(3) replace
esttab m* using "`here'/output/table_csdid_att_ipw.txt", mtitle b(3) se(3) replace
