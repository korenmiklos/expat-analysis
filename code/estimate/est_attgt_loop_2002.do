...
cap log close
log using "`here'/output/est_attgt_loop_2002", text replace
...
foreach depvar in TFP_cd lnIK_0 lnQh lnQhr {
	foreach var in foreign foreign_hire has_expat {
		attgt `depvar' if year >= 2002, treatment(`var') aggregate(e) pre(5) post(5) reps(20) notyet limitcontrol(foreign == 0)
		count if e(sample) == 1
		eststo model_`sample'`var'`depvar', title("`depvar' `sample' `var'")
	}
}

esttab model* using "`here'/output/table_attgt_loop_2002.tex", mtitle b(3) se(3) replace
esttab model* using "`here'/output/table_attgt_loop_2002.txt", mtitle b(3) se(3) replace
